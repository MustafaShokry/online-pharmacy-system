...
export default function FetchCartProvider(props) {
    ...
    const AddProductToCart = async (productId) => {
        try {
            await axios.post(`${BaseUrl}/cart/items`, { productId }, {
                headers: {
    ...
    const deleteProductCart = async (productId) => {
        try {
            await axios.delete(`${BaseUrl}/cart/items/${productId}`, {
                headers: {
                    Authorization: `Bearer ${token}` // Add Authorization header
                }
            });
    ...
    const UpdateProductCart = async (productId, quantity) => {
        try {
            await axios.put(`${BaseUrl}/cart/items/${productId}`, { productId ,quantity }, {
                headers: {
                    Authorization: `Bearer ${token}` // Add Authorization header
                }
            });
